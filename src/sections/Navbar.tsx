    ...
    <>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <motion.a variants={itemVariants} href="#services" onClick={handleLinkClick} className="text-3xl font-light text-bone uppercase tracking-widest hover:text-accent transition-colors">
              Services
            </motion.a>
            <motion.a variants={itemVariants} href="#projects" onClick={handleLinkClick} className="text-3xl font-light text-bone uppercase tracking-widest hover:text-accent transition-colors">
              Projects
            </motion.a>
            <motion.a variants={itemVariants} href="#history" onClick={handleLinkClick} className="text-3xl font-light text-bone uppercase tracking-widest hover:text-accent transition-colors">
              Our Legacy
            </motion.a>
            <motion.a variants={itemVariants} href="#contact" onClick={handleLinkClick} className="text-3xl font-light text-bone uppercase tracking-widest hover:text-accent transition-colors">
              Inquire
            </motion.a>
      ...
      <motion.nav
        ...
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-6xl rounded-full transition-all duration-300 ${
          scrolled
            ? "bg-background/80 border border-foreground/10 backdrop-blur-md shadow-2xl px-6 md:px-8"
            : "bg-transparent px-4 md:px-6"
        }`}
      >
        {/* Forced height constraint prevents nav bar size from changing when logo scales */}
        <div className="flex items-center justify-between h-16 md:h-20">
          
          {/* Logo - Scales up visually but is constrained by parent height */}
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <button className="hidden md:block bg-accent hover:bg-accent/90 text-white font-medium text-xs uppercase tracking-wider px-5 py-2.5 rounded-full transition-colors cursor-pointer">
              Inquire
            </button>
