...
function TiltCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Invert Y axis for natural tilt
            ...
            >
              <TiltCard className="group relative h-120 rounded-2xl cursor-pointer overflow-hidden border border-border shadow-lg hover:shadow-2xl hover:shadow-accent/10 transition-shadow duration-500">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 z-20" style={{ transform: 'translateZ(40px)' }}>
                  <span className="text-accent text-[10px] uppercase tracking-[0.2em] font-mono mb-2 block">
                    {service.category}
                  </span>
                  <h3 className="text-2xl font-light tracking-wide text-bone mb-3 group-hover:text-accent transition-colors duration-300">
                    {service.title}
                  </h3>
                  <p className="text-bone/60 text-sm font-light leading-relaxed opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500 ease-out">
                    {service.description}
                  </p>
                </div>
